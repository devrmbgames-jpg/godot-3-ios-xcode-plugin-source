...
didReceiveNotificationResponse:(UNNotificationResponse *)response
    ...
    NSLog(@"FirebaseMSG tapped: %@ %@", title, userInfo.description);
    completionHandler();
    FirebaseCloudMessaging* instance = FirebaseCloudMessaging::get_instance();
    if (instance) {
        instance->message_received(from_nsdictionary(userInfo));
...
void FirebaseCloudMessaging::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("setup"), &FirebaseCloudMessaging::setup);
    ClassDB::bind_method(D_METHOD("push_local"), &FirebaseCloudMessaging::push_local);
    
    ADD_SIGNAL(MethodInfo("token"));
    ADD_SIGNAL(MethodInfo("message"));
    ADD_SIGNAL(MethodInfo("tapped_message", PropertyInfo(Variant::STRING, "TITLE")));
...
void FirebaseCloudMessaging::push_local(String title, String msg) {
    NSLog(@"[NOTIFICATION] push local");
    // Configure the notification's payload.
    UNMutableNotificationContent *content = [[UNMutableNotificationContent alloc] init];
    content.title = [NSString stringWithUTF8String:title.utf8()];
    content.body = [NSString stringWithUTF8String:msg.utf8()];
